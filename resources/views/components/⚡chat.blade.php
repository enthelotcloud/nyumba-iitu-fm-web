...
new class extends Component {
    ...
    {
        $this->name = session('chat_name', '');
        $this->phone = session('chat_phone', '');
        $this->loadChats();
    }

    public function loadChats()
    {
        $this->chats = ChatModel::latest()->take(50)->get()->reverse();
    }
...
<div class="h-full w-full flex flex-col bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden" wire:poll.5s="loadChats" style="font-family: 'Poppins', sans-serif;">
    ...
    <div class="bg-blue-600 text-white px-5 py-4 flex items-center justify-between shrink-0 border-b border-blue-700">
        ...
        <span class="text-[10px] uppercase tracking-wider font-bold bg-white text-blue-600 px-2.5 py-1 rounded-full shadow-sm flex items-center">
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 flex flex-col">
        @forelse($chats as $chat)
            <div class="flex flex-col w-full {{ $chat->session_id === session()->getId() ? 'items-end' : 'items-start' }}">
                <div class="flex items-baseline gap-2 mb-1 px-1">
                    <span class="text-xs font-semibold text-slate-600">
                        {{ $chat->session_id === session()->getId() ? 'You' : $chat->name }}
                    </span>
                    <span class="text-[9px] text-slate-400">{{ $chat->created_at->format('H:i') }}</span>
                </div>
        ...
        @empty
            <div class="h-full flex flex-col items-center justify-center text-slate-400 space-y-3 p-4 text-center">
                ...
                <p class="text-sm font-medium">Be the first to send a request!</p>
            </div>
        @endforelse
    </div>
        ...
        <form wire:submit.prevent="sendMessage" class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2 sm:gap-3">
                    <div>
                        <input type="text" wire:model.defer="name" placeholder="Name"
                               class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-slate-50 focus:bg-white transition-all" required>
                    </div>
                    <div>
            ...
            <div class="flex gap-2">
                <input type="text" wire:model.defer="message" placeholder="Message studio..."
                       class="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-slate-50 focus:bg-white transition-all" required>

                <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl transition-all hover:scale-105 active:scale-95 shadow-md shadow-blue-600/20 flex items-center justify-center shrink-0 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
